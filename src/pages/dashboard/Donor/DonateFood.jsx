...
/* ---------------- Header ---------------- */
function DashboardHeader({ onMenuClick, name }) {
  return (
...
function DonateFoodForm({ form, setForm, onSubmit, submitting, success, error }) {
  ...
  return (
    ...
    >
      {success && (
        <div className="flex items-center gap-2 rounded-xl bg-emerald-50 border border-emerald-100 px-4 py-3 mb-6 text-sm text-emerald-700">
          <CheckCircle2 className="h-4 w-4 flex-shrink-0" aria-hidden="true" />
          Donation posted successfully! NGOs nearby can now see it.
        </div>
      )}
      {error && (
        <div className="flex items-center gap-2 rounded-xl bg-red-50 border border-red-100 px-4 py-3 mb-6 text-sm text-red-600">
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        ...
        <span
          className={`inline-block rounded-full px-3 py-1 text-xs font-bold uppercase ${isComplete ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"
            }`}
        >
  ...
  async function handleSubmit(e) {
    ...
    try {
      await addDoc(collection(db, "donations"), {
        ...
        window: form.window,
        expiry: form.expiry,
        status: "Available", // Available -> Claimed -> In Transit -> Delivered
  ...
  return (
    <div className="min-h-screen bg-gray-50/60 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <DashboardHeader onMenuClick={() => setSidebarOpen(true)} name={profile?.name?.split(" ")[0] || "Donor"} />
